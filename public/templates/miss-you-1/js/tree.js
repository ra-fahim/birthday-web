...
function cherryColor() {
  return CHERRY_COLORS[randomInt(0, CHERRY_COLORS.length - 1)];
}
...
class Bloom {
  constructor(tree, point, figure, color = cherryColor(),
    alpha = randomFloat(0.5, 1), angle = randomFloat(0, Math.PI * 2), scale = 0.1) {
    this.tree = tree;
    this.point = point;
    this.color = color;
    this.alpha = alpha;
...
class Tree {
  ...
  createBloom(figure) {
    ...
    const rx = this.width * 0.40;
    const ry = this.height * 0.33;
    ...
    for (let attempt = 0; attempt < maxAttempts; attempt++) {
      ...
      const dx = (x - cx) / rx;
      const dy = (y - cy) / ry;
      if (inCanopy(dx, dy, 1, 1)) {
        return new Bloom(this, new Point(x, y), figure, cherryColor());
      }
    }
    return new Bloom(this, new Point(cx, cy), figure, cherryColor());
  }
  ...
  createFallingBloom() {
    const figure = this.seed.heart.figure;
    const crown = {
      x: this.width / 2 - 20 + getTreeShiftX() * 0.3,
      y: this.height * 0.42
    };
    const bloom = new Bloom(
      this,
      new Point(crown.x + randomFloat(-180, 180), crown.y + randomFloat(-150, 80)),
      figure,
      cherryColor(),
      randomFloat(0.65, 1),
      randomFloat(-0.8, 0.8),
