...
export default function DeleteConfirmModal({ label='Delete', title='Delete this website?', description='This action permanently removes the website and its uploaded media.', confirmText='Delete permanently', onConfirm }: { label?:string; title?:string; description?:string; confirmText?:string; onConfirm:()=>Promise<void>|void }) {
  ...
  return <>
    ...
    {open && typeof document!=='undefined' && createPortal(<div className="confirm-backdrop" role="presentation" onMouseDown={e=>{if(!busy&&e.target===e.currentTarget)setOpen(false)}}>
      <div className="confirm-modal" role="dialog" aria-modal="true" aria-labelledby="delete-title" aria-describedby="delete-description" onMouseDown={e=>e.stopPropagation()}>
        ...
        <div className="confirm-icon"><AlertTriangle size={22}/></div><h2 id="delete-title">{title}</h2><p id="delete-description">{description}</p>
        ...
        <div className="confirm-actions"><button type="button" className="confirm-cancel" disabled={busy} onClick={()=>setOpen(false)}>Cancel</button><button type="button" className="confirm-delete" disabled={busy} onClick={confirm}>{busy?'Deleting…':confirmText}</button></div>
      </div>
    </div>,document.body)}
  </>;
}
