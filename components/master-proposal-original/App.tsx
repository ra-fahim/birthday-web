...
const App: React.FC<{ content?: BirthdayContent }> = () => {
  const content = useProposalContent();
  const storyData = content?.proposalMasterStory?.length ? content.proposalMasterStory : DEFAULT_STORY_DATA;
  ...
  const heroTitle = content?.proposalMasterHeroTitle || 'To My Dearest';
  const scrollLabel = content?.proposalMasterScrollLabel || 'Scroll slowly';
  ...
  useEffect(() => {
    ...
    Object.entries(themeColors).forEach(([key, value]) => { root.style.setProperty(key, value as string); });
