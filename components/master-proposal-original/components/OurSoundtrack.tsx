...
import { getYouTubeId, getYouTubePlaylistId, loadYouTubeIframeAPI, fetchYouTubeOEmbed } from '../utils/youtube';
...
export const OurSoundtrack: React.FC<OurSoundtrackProps> = ({ isIntroComplete = true }) => {
  ...
  const configuredSource = content?.musicUrl?.trim() || '';
  const configuredSongs: Song[] = configuredSource ? [{ id:'configured', title: content?.proposalMasterSoundtrackTitle || 'Our Soundtrack', artist: 'Your playlist', albumArt: '', note: content?.proposalMasterSoundtrackSubtitle || 'Songs that sound like us.', audioUrl: configuredSource }] : SONGS;
  const [currentSong, setCurrentSong] = useState<Song>(() => configuredSongs[0]);
  ...
  const handleYouTubeStateChange = (event: any) => {
    if (event.data === 1) {
      ...
      setCurrentSong((prev) => {
        const idx = configuredSongs.findIndex((s) => s.id === prev.id);
        return idx < configuredSongs.length - 1 ? configuredSongs[idx + 1] : configuredSongs[0];
      });
      setIsPlaying(true);
    }
  };

  const getOrCreateYouTubePlayer = async (sourceIdOrUrl: string): Promise<any> => {
    const playlistId = getYouTubePlaylistId(sourceIdOrUrl);
    const videoId = getYouTubeId(sourceIdOrUrl) || undefined;
    await loadYouTubeIframeAPI();
    if (ytPlayerRef.current) {
      if (playlistId) {
        if (loadedYtIdRef.current !== playlistId) { ytPlayerRef.current.loadPlaylist({list: playlistId, listType: 'playlist'}); loadedYtIdRef.current = playlistId; }
      } else if (videoId && loadedYtIdRef.current !== videoId) {
        ytPlayerRef.current.cueVideoById(videoId);
        loadedYtIdRef.current = playlistId || videoId || null;
      }
    ...
    return new Promise((resolve) => {
      const player = new (window as any).YT.Player(ytContainerRef.current, {
        height: '0',
        width: '0',
        ...(playlistId ? { playerVars: { autoplay: 0, controls: 0, disablekb: 1, playsinline: 1, listType: 'playlist', list: playlistId } } : { videoId, playerVars: { autoplay: 0, controls: 0, disablekb: 1, playsinline: 1 } }),
        events: {
          onReady: () => {
  ...
  useEffect(() => { if (currentSong.id !== configuredSongs[0]?.id) setCurrentSong(configuredSongs[0]); }, [configuredSource]);

  const currentIndex = configuredSongs.findIndex((s) => s.id === currentSong.id);
  const hasPrevious = currentIndex > 0;
  const hasNext = currentIndex < configuredSongs.length - 1 && currentIndex >= 0;
  ...
  useEffect(() => {
    ...
    (async () => {
      const entries = await Promise.all(
        configuredSongs.filter((song) => getYouTubeId(song.audioUrl) && (!song.title || !song.artist || !song.albumArt))
          .map(async (song) => {
  ...
  const goToPrevious = () => {
    if (currentIndex <= 0) {
      setCurrentSong(configuredSongs[configuredSongs.length - 1]);
    } else {
      setCurrentSong(configuredSongs[currentIndex - 1]);
    }
    setIsPlaying(true);
    setError(false);
  };
  ...
  const goToNext = () => {
    if (currentIndex >= configuredSongs.length - 1) {
      setCurrentSong(configuredSongs[0]);
    } else {
      setCurrentSong(configuredSongs[currentIndex + 1]);
    }
    setIsPlaying(true);
  ...
  const togglePlay = async () => {
    ...
    const ytPlaylistId = getYouTubePlaylistId(currentSong.audioUrl);
    const spotifyMatch = currentSong.audioUrl.match(/https?:\/\/open\.spotify\.com\/(playlist|track|album)\/([A-Za-z0-9]+)/);
    if (ytPlaylistId) {
      const player = await getOrCreateYouTubePlayer(currentSong.audioUrl);
      if (isPlaying) player.pauseVideo(); else { setError(false); player.playVideo(); }
      return;
    }
    if (spotifyMatch) {
      window.open(`https://open.spotify.com/${spotifyMatch[1]}/${spotifyMatch[2]}`, '_blank', 'noopener,noreferrer');
      return;
    }
    if (ytId) {
  ...
  useEffect(() => {
    const ytId = getYouTubeId(currentSong.audioUrl);
    const ytPlaylistId = getYouTubePlaylistId(currentSong.audioUrl);
    const isSpotify = currentSong.audioUrl.includes('open.spotify.com/');
      ...
      <div ref={sectionRef} className="w-full max-w-5xl mx-auto px-6 py-10">
        <div className="text-center mb-16">
          <h2 className="font-serif text-3xl md:text-5xl mb-4 text-love-text dark:text-love-dark-text">
            {content?.proposalMasterSoundtrackTitle || 'Our Soundtrack'}
          </h2>
          <p className="text-love-accent dark:text-love-dark-accent/80 text-sm tracking-wide uppercase">
            {content?.proposalMasterSoundtrackSubtitle || 'Songs that tell our story'}
          </p>
        </div>
        ...
        <div className="flex flex-col lg:flex-row items-center gap-12 lg:gap-20">
          ...
          {/* Hidden Audio Element — inactive when the source is a YouTube/Spotify playlist */}
      <audio
            ref={audioRef}
            preload="none"
              ...
              <div className="flex flex-col items-center gap-4">
                ...
                {error && (
                  ...
                  >
                    ...
                    <span>File missing or link broken — use a direct audio URL, YouTube link/playlist, or Spotify playlist link</span>
                  </motion.div>
                )}
              </div>
          ...
          <div className="flex-1 w-full max-w-md min-w-0">
            ...
            >
              ...
              {currentSong.audioUrl.includes('open.spotify.com/') && (
                <iframe
                  className="w-full h-[152px] rounded-xl mb-4"
                  src={currentSong.audioUrl.replace('open.spotify.com/', 'open.spotify.com/embed/')}
