...
const SILLY_PROMPTS = [
  { title: "I made this just for you.", subtitle: "" },
...
export const IntroGate: React.FC<IntroGateProps> = ({ onComplete }) => {
  ...
  const introTitle = content?.proposalMasterIntroTitle || 'Will you be my Valentine?';
  const introSubtitle = content?.proposalMasterIntroSubtitle || '...and for a lifetime?';
              ...
              <AnimatePresence mode="wait">
                ...
                >
                  <h1 className="font-serif text-3xl md:text-5xl lg:text-6xl text-love-text dark:text-love-dark-text italic mb-4 leading-tight px-4">
                    {rejectionCount === 0 ? introTitle : currentPrompt.title}
                  </h1>
                  <p className="text-lg md:text-xl text-love-text/60 dark:text-love-dark-text/60 font-light">
                    {rejectionCount === 0 ? introSubtitle : currentPrompt.subtitle}
                  </p>
                </motion.div>
              </AnimatePresence>
            </div>
