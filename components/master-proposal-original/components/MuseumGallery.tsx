...
export const MuseumGallery: React.FC = () => {
  ...
  const galleryTitle = content?.proposalMasterMuseumTitle || 'Museum of Our Love';
  const gallerySubtitle = content?.proposalMasterMuseumSubtitle || 'A Curated Collection of Us';
  const items = content?.gallery?.length ? content.gallery.map((g:any, i:number): MuseumItem => {
    const url = String(g.url || '');
    const isVideo = /\.(mp4|webm|mov)(\?|$)/i.test(url) || /youtu(?:\.be|be\.com)/i.test(url);
    return { id: String(i+1), type: isVideo ? 'video' : 'image', url, title: g.caption || `Memory ${i+1}`, date: '', description: g.caption || '' };
  }) : MUSEUM_ITEMS;
